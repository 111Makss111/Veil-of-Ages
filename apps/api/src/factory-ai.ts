...
export function buildShortsStoryPlan(releaseKey:string,title:string,recipe:Record<string,unknown>={}):ShortsStoryPlan{
  ...
  const scenes=beats.map((beat,position)=>{
    ...
    const videoPrompt=`Generate a ${position===2?'10–12':'6–10'} second vertical 9:16 CINEMATIC VIDEO CLIP, not a still image, for scene ${position+1} of 3 of the original Viking song “${title}”. ${continuity} ACTION: ${beat.moment} CAMERA: ${beat.motion} The clip must have a readable beginning, continuous physical movement and a clear end pose that can cut into the next scene. Animate wind-driven snow, cloth, hair, rope and lantern light with natural weight; preserve facial identity and anatomy. Epic Nordic cinematic realism, grounded historical materials, no talking or lip-sync required, no text or lyrics on screen. No frozen slideshow, no album-cover pose, no random new characters, no face morphing, no extra limbs, no modern objects, no fantasy armor, no logo, no watermark, no subtitles, no border.`;
    const prompt=`Create a coherent vertical 9:16 keyframe for scene ${position+1} of a 30-second Veil of Ages micro-story for the original song “${title}”. ${continuity} Story moment: ${beat.moment} ${beat.motion} Epic Nordic cinematic realism, emotionally specific adult Viking man and adult Viking woman, historically inspired wool, leather, iron and weathered timber, premium atmospheric depth. VERTICAL 9:16 COMPOSITION: keep important faces and hands in the central 60%. No readable text, logo, watermark, border, duplicate people, celebrity likeness or modern objects.`;
