...
test('factory browser script parses and storage fails closed without configuration',()=>{
  ...
  assert.match(factoryScript,/Кінетичний текст/);
  assert.match(factoryScript,/Розпізнаємо вокал/);
...
test('factory: durable library, quotas, duplicates, reservation, retry, review and private upload',async()=>{
  ...
  try{
    ...
    const storyboardStart=await post('/api/factory/releases/'+id+'/shorts-storyboard',{});assert.equal(storyboardStart.statusCode,202,storyboardStart.body);
    for(let i=0;i<100;i++){const row=(await q('SELECT short_plan FROM factory_releases WHERE id=$1',[id])).rows[0] as {short_plan:{storyboard?:{state:string}}};if(row.short_plan.storyboard?.state==='ready')break;if(i===99)assert.fail('Expected storyboard ready state');await new Promise(resolve=>setTimeout(resolve,10));}
    assert.equal(generated,generatedBeforePlan+6);assert.equal((await q("SELECT a.id FROM factory_short_scenes s JOIN factory_assets a ON a.id=s.asset_id WHERE s.release_id=$1 AND a.state='ready'",[id])).rows.length,6);
    ...
    assert.equal(renderFormats.at(-1),'shorts');assert.equal(renderSceneCounts.at(-1),6);assert.equal(lyricOverlayCounts.at(-1),2);assert.deepEqual(imageFormats.slice(-6),['portrait','portrait','portrait','portrait','portrait','portrait']);
    const shortRow=(await q('SELECT short_cover_id,short_plan FROM factory_releases WHERE id=$1',[id])).rows[0] as {short_cover_id:string|null;short_plan:{kineticText:{mode:string;cues:unknown[]}}};assert.ok(shortRow.short_cover_id);assert.equal(shortRow.short_plan.kineticText.mode,'story-captions');assert.equal(shortRow.short_plan.kineticText.cues.length,2);
    ...
    assert.equal(generated,generatedBefore+6);assert.deepEqual(imageFormats.slice(-6),['portrait','portrait','portrait','portrait','portrait','portrait']);
