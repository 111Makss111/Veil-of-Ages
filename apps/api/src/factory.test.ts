...
test('factory: durable library, quotas, duplicates, reservation, retry, review and private upload',async()=>{
  ...
  const objects=new Map<string,Buffer>();let otherBytes=0,puts=0,deletes=0,failPut=false,renderFail=true,renders=0,generated=0;const renderPresets:string[]=[],renderSceneCounts:number[]=[],renderFormats:string[]=[],imageFormats:string[]=[],lyricOverlayCounts:number[]=[],lyricTrackCueCounts:number[]=[];
  ...
  try{
    ...
    const automaticShorts=await post('/api/factory/releases/'+id+'/shorts',{chorusStart:44});assert.equal(automaticShorts.statusCode,202,automaticShorts.body);await waitShortState(id,'review');assert.equal(renderSceneCounts.at(-1),1);assert.equal(lyricOverlayCounts.at(-1),0);assert.ok((lyricTrackCueCounts.at(-1)||0)>=2);const timedShort=(await q('SELECT short_plan FROM factory_releases WHERE id=$1',[id])).rows[0] as {short_plan:{kineticText:{mode:string;clipStart:number}}};assert.equal(timedShort.short_plan.kineticText.mode,'manual');assert.equal(timedShort.short_plan.kineticText.clipStart,44);
